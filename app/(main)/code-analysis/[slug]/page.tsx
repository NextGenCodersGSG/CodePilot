...
const Page = ({ params }: IParams) => {
  ...
  const getSeverityColor = (severity: string) => {
    switch (severity.toLowerCase()) {
      ...
      default:
        return "bg-[#00406C] hover:bg-[#003A61]"
    }
  }
      ...
      <div className="container mx-auto p-6">
        <Card className="bg-[#001523] border-[#002945]">
          <CardContent className="pt-6 flex flex-col items-center justify-center min-h-[300px]">
            <Info className="h-16 w-16 text-[#00406C] mb-4" />
            <h2 className="text-2xl font-bold text-[#F2F2F2] mb-2">Project Not Found</h2>
            <p className="text-[#B3B3B3]">The project with slug "{slug}" could not be found.</p>
          </CardContent>
        </Card>
      </div>
    )
        ...
        <Card className="bg-[#001523] border-[#002945]">
          <CardHeader>
            <div className="flex items-center gap-3">
              <FileCode className="h-6 w-6 text-[#00406C]" />
              <div>
                <CardTitle className="text-2xl text-[#F2F2F2]">{project.name || "Project Analysis"}</CardTitle>
                <CardDescription className="text-[#B3B3B3]">Analysis results and recommendations</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            <div className="bg-[#001A2C] p-4 rounded-lg border border-[#002945]">
              <h3 className="text-lg font-medium text-[#F2F2F2] mb-2">Project Description</h3>
              <p className="text-[#B3B3B3]">{project.description || "No description available"}</p>
            </div>
          </CardContent>
        </Card>
      </motion.div>
      ...
      >
        <Tabs defaultValue="all" className="w-full">
          ...
          <TabsContent value="all" className="space-y-6">
            {/* Bugs Card */}
            <Card className="bg-[#001523] border-[#002945] overflow-hidden pt-0">
              <div className="h-3 bg-gradient-to-r from-red-400 to-red-800"></div>
              <CardHeader className="flex flex-row items-center justify-between">
                <div className="flex items-center gap-2">
                  <AlertTriangle className="h-5 w-5 text-red-500" />
                  <CardTitle className="text-[#F2F2F2]">Bugs & Errors</CardTitle>
                </div>
                <Badge className="bg-[#001A2C] text-red-500">{project.bugs.length}</Badge>
              </CardHeader>
              <CardContent>
                  ...
                  {project.bugs.length > 0 ? (
                    <div className="space-y-4">
                      {project.bugs.map((bug, index) => (
                        <div key={index} className="bg-[#001A2C] p-4 rounded-lg border border-[#002945]">
                          <div className="flex items-center justify-between mb-2">
                            <h3 className="font-medium text-[#F2F2F2]">{bug.error}</h3>
                            <Badge className={`${getSeverityColor(bug.severity)}`}>{bug.severity}</Badge>
                          </div>
                          {bug.correction && (
                            <div className="mt-2 text-sm text-[#B3B3B3]">
                              <span className="font-medium text-[#F2F2F2]">Suggested Fix: </span>
                              {bug.correction}
                            </div>
                          )}
                        </div>
                    ...
                    <div className="flex flex-col items-center justify-center h-[150px] text-[#B3B3B3]">
            ...
            <Card className="bg-[#001523] border-[#002945] overflow-hidden pt-0">
              ...
              <CardHeader className="flex flex-row items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-[#F2F2F2]">Security Vulnerabilities</CardTitle>
                </div>
                <Badge className="bg-[#001A2C] text-green-500">{project.security_issues.length}</Badge>
              </CardHeader>
              <CardContent>
                <ScrollArea className="h-[200px] pr-4">
                  {project.security_issues.length > 0 ? (
                    <div className="space-y-4">
                      {project.security_issues.map((issue, index) => (
                        <div key={index} className="bg-[#001A2C] p-4 rounded-lg border border-[#002945]">
                          <div className="flex items-center justify-between mb-2">
                            <h3 className="font-medium text-[#F2F2F2]">{issue.vulnerability}</h3>
                            <Badge className={`${getSeverityColor(issue.severity)}`}>{issue.severity}</Badge>
                          </div>
                          {issue.fix && (
                            <div className="mt-2 text-sm text-[#B3B3B3]">
                              <span className="font-medium text-[#F2F2F2]">Recommended Fix: </span>
                              {issue.fix}
                            </div>
                          )}
                        </div>
            ...
            <Card className="bg-[#001523] border-[#002945] overflow-hidden pt-0">
              ...
              <CardHeader className="flex flex-row items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-[#F2F2F2]">Performance Issues</CardTitle>
                </div>
                <Badge className="bg-[#001A2C] text-yellow-500">{project.performance_issues.length}</Badge>
              </CardHeader>
              <CardContent>
                <ScrollArea className="h-[200px] pr-4">
                  {project.performance_issues.length > 0 ? (
                    <div className="space-y-4">
                      {project.performance_issues.map((issue, index) => (
                        <div key={index} className="bg-[#001A2C] p-4 rounded-lg border border-[#002945]">
                          <div className="flex items-center justify-between mb-2">
                            <h3 className="font-medium text-[#F2F2F2]">{issue.issue}</h3>
                            <Badge className={`${getSeverityColor(issue.severity)}`}>{issue.severity}</Badge>
                          </div>
                          {issue.solution && (
                            <div className="mt-2 text-sm text-[#B3B3B3]">
                              <span className="font-medium text-[#F2F2F2]">Solution: </span>
                              {issue.solution}
                            </div>
                          )}
                        </div>
            ...
            <Card className="bg-[#001523] border-[#002945] overflow-hidden pt-0">
              ...
              <CardHeader className="flex flex-row items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-[#F2F2F2]">Overall Suggestions</CardTitle>
                </div>
                <Badge className="bg-[#001A2C] text-blue-500">{project.overall_suggestions.length}</Badge>
              </CardHeader>
              <CardContent>
                <ScrollArea className="h-[200px] pr-4">
                      ...
                      {project.overall_suggestions.map((suggestion, index) => (
                        <div key={index} className="bg-[#001A2C] p-4 rounded-lg border border-[#002945]">
                          <div className="flex items-start gap-3">
                            ...
                            <p className="text-[#F2F2F2]">{suggestion}</p>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <CardContent>
                {project.bugs.length > 0 ? (
                      ...
                      <div key={index} className="bg-[#001A2C] p-4 rounded-lg border border-[#002945]">
                        ...
                        {bug.correction && (
                          <div className="mt-4 text-sm">
                            <div className="font-medium text-[#F2F2F2] mb-1">Suggested Fix:</div>
                            <p className="text-[#B3B3B3]">{bug.correction}</p>
                          </div>
                        )}
                      </div>
                    ))}
                  ...
                  <div className="flex flex-col items-center justify-center py-12 text-[#B3B3B3]">
                    ...
                    <h3 className="text-xl font-medium text-[#F2F2F2] mb-2">No Bugs Detected</h3>
              ...
              <CardContent>
                {project.security_issues.length > 0 ? (
                  <div className="space-y-4">
                    {project.security_issues.map((issue, index) => (
                      <div key={index} className="bg-[#001A2C] p-4 rounded-lg border border-[#002945]">
                        ...
                        {issue.fix && (
                          <div className="mt-4 text-sm">
                            <div className="font-medium text-[#F2F2F2] mb-1">Recommended Fix:</div>
                            <p className="text-[#B3B3B3]">{issue.fix}</p>
                          </div>
                        )}
                      </div>
                    ))}
                  ...
                  <div className="flex flex-col items-center justify-center py-12 text-[#B3B3B3]">
                    ...
                    <h3 className="text-xl font-medium text-[#F2F2F2] mb-2">No Security Vulnerabilities</h3>
              ...
              <CardContent>
                {project.performance_issues.length > 0 ? (
                  <div className="space-y-4">
                    {project.performance_issues.map((issue, index) => (
                      <div key={index} className="bg-[#001A2C] p-4 rounded-lg border border-[#002945]">
                        ...
                        {issue.solution && (
                          <div className="mt-4 text-sm">
                            <div className="font-medium text-[#F2F2F2] mb-1">Solution:</div>
                            <p className="text-[#B3B3B3]">{issue.solution}</p>
                          </div>
                        )}
                      </div>
                    ))}
                  ...
                  <div className="flex flex-col items-center justify-center py-12 text-[#B3B3B3]">
                    ...
                    <h3 className="text-xl font-medium text-[#F2F2F2] mb-2">No Performance Issues</h3>
